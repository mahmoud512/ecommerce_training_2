                    ...
                    $user_id = $_SESSION['user_id'];
                      ...
                      while ($row = $result->fetch_assoc()) {
                        ...
                        ?>
                          <tr>
                            <?php 
                              $sql_pro = "SELECT * FROM `products` WHERE id = $por_id";
                              $result_pro = $conn->query($sql_pro);
                              $row_pro = $result_pro->fetch_assoc();
                            ?>
                      <th class="pl-0 border-0" scope="row">
                        <div class="media align-items-center"><a class="reset-anchor d-block animsition-link" href="detail.php?id_pro=<?=$row['id_pro']?>"><img style="width: 80px; height: 100px;" src="../admin/assets/img/upload/<?php
                        $sql_cover = "SELECT * FROM `product_imges` WHERE id_pro = $por_id";
                        $result_cover = $conn->query($sql_cover);
                        $row_cover = $result_cover->fetch_assoc();
                        echo $row_cover['img_names'];
                        ?>" alt="..." width="70"/></a>
                          <div class="media-body ml-3"><strong class="h6"><a class="reset-anchor animsition-link" href="detail.php"><?=$row_pro['name']?></a></strong></div>
                        </div>
                      </th>
                      ...
                      <td class="align-middle border-0">
                        <div class="border d-flex align-items-center justify-content-between px-3"><span class="small text-uppercase text-gray headings-font-family">Quantity</span>
                          <div class="quantity">
                            <button class="dec-btn p-0"><i class="fas fa-caret-left"></i></button>
                            <input class="form-control form-control-sm border-0 shadow-0 p-0" type="text" value="<?=$row['quantity']?>"/>
                            <button class="inc-btn p-0"><i class="fas fa-caret-right"></i></button>
                          </div>
                        </div>
                      </td>
